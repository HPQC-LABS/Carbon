    ...
    supplemented with auxiliary log functions, and ECG($N$) stands for  explicitly correlated Gaussian ansatz with $N$ optimizable parameters. Numbers in parentheses are estimated uncertainties \emph{within} the method used, so for FN-DMC does not include fixed-node error and for FCIQMC does not include basis set error. No numbers were obtained with basis set extrapolations.}
    ...
    \multicolumn{2}{l}{\hspace{-0.75mm}Breit \& QED} & -0&5\multicolumn{2}{c}{} & \tabularnewline
     DBOC &  & -0&270(004) & \multicolumn{2}{c}{}\tabularnewline
    \noalign{\vskip2mm}
    \hline
    \noalign{\vskip2mm}
    Total (theory)  & Present & \textcolor{red} {90~832}&\textcolor{red}{244} & \qquad  -0&777\tabularnewline[2mm]
    \hline
    \noalign{\vskip2mm}
    Experiment   & 2017 \cite{Haris2017}& 90~833&021(009)  & \multicolumn{2}{c}{}\tabularnewline[2mm]
    \hline
    \noalign{\vskip2mm}
    ...
    Diagonal Born-Oppenheimer breakdown corrections (DBOC) to the clamped nucleus approximation were calculated using $\tt{CFOUR}$, with $\tt{MRCC}$ used for the coupled-cluster part. Our value of -0.27\,cm$^{-1}$ is more than triple the value of -0.08\,cm$^{-1}$ estimated in \cite{Klopper2010}, due to including higher levels of correlation.
